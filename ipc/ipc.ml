...
let get_leuint64 str off =
  if Sys.big_endian
  then swap64 (get_uint64 str off)
  else get_uint64 str off

let set_leuint64 buf off v =
  if Sys.big_endian
  then set_uint64 buf off (swap64 v)
  else set_uint64 buf off v
...
and go fd tmp off len =
  ...
  else let len' = Unix.read fd tmp off len in
...
and go fd tmp off len =
  if len > 0
  then let len' = Unix.write fd tmp off len in
       go fd tmp (off + len') (len - len')
