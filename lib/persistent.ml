...
type 'a t =
  ...
  | Compare_exchange : rdwr Addr.t * (atomic, 'a) value * 'a Atomic.t * 'a * bool -> bool t
...
let pp : type a. a t fmt = fun ppf v ->
  ...
  | Atomic_set (addr, v, x) ->
     pf ppf "atomic_set %016x (%a : %a)" (addr :> int) (pp_of_value v) x pp_value v
  | Fetch_add (addr, v, x) ->
     pf ppf "fetch_add  %016x (%a : %a)" (addr :> int) (pp_of_value v) x pp_value v
  | Fetch_or (addr, v, x) ->
     pf ppf "fetch_or   %016x (%a : %a)" (addr :> int) (pp_of_value v) x pp_value v
  | Fetch_sub (addr, v, x) ->
     pf ppf "fetch_sub  %016x (%a : %a)" (addr :> int) (pp_of_value v) x pp_value v
  ...
  | Persist (addr, len) ->
     pf ppf "persist    %016x (%d)" (addr :> int) len
  | Compare_exchange (addr, v, x, y, weak) ->
    pf ppf "compare_exchange weak:%b %016x (%a : %a) (%a : %a)" weak (addr :> int)
       (pp_of_value v) (Atomic.get x) pp_value v
...
let ralloc mmu ~kind len payloads =
  ...
  then ( let time =
           ...
           then 0 (* TODO(dinosaure): check that! Why we don't use [Unix.getppid]
                     to note the owner of this node? *)
           else older_reader mmu in
         blitv payloads mmu.memory brk
       ; atomic_set_leuintnat mmu.memory 0 (brk + len)
       ; if kind = `Node then atomic_set_leuintnat mmu.memory (brk + _header_owner) time
...
let alloc mmu ~kind len payloads =
  ...
  try match Hashtbl.find mmu.free len with
    ...
    | cell :: tl ->
      ...
      if kind = `Node then atomic_set_leuintnat mmu.memory (cell.addr + _header_owner) time ;
...
let rec run : type c a. c mmu -> a t -> a = fun ({ memory; _ } as mmu) cmd ->
  let () = match cmd with
    | Bind _ | Return _ -> ()
    | cmd -> Log.debug (fun m -> m "%a" S.pp cmd) in
  match cmd with
  | Atomic_get (addr, Int8) ->
     atomic_get_uint8 memory (addr :> int)
  | Atomic_set (addr, Int8, v) ->
     atomic_set_uint8 memory (addr :> int) v
  | Atomic_get (addr, LEInt) ->
     atomic_get_leuintnat memory (addr :> int)
  | Atomic_set (addr, LEInt, v) ->
     atomic_set_leuintnat memory (addr :> int) v
  | Atomic_get (addr, LEInt16) ->
     atomic_get_leuint16 memory (addr :> int)
  | Atomic_set (addr, LEInt16, v) ->
     atomic_set_leuint16 memory (addr :> int) v
  | Atomic_get (addr, LEInt31) ->
     atomic_get_leuint31 memory (addr :> int)
  | Atomic_set (addr, LEInt31, v) ->
     atomic_set_leuint31 memory (addr :> int) v
  | Atomic_get (addr, LEInt64) ->
     atomic_get_leuint64 memory (addr :> int)
  | Atomic_set (addr, LEInt64, v) ->
     atomic_set_leuint64 memory (addr :> int) v
