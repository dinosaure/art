...
let[@coverage off] pp_n4 ppf keys =
  Fmt.pf ppf "%a" Fmt.(Dump.array pp_char) (Array.init 4 (fun i -> keys.!{i}))
...
let[@coverage off] pp_keys : type a. kind:a kind -> a Fmt.t =
 fun ~kind ->
  match kind with
  | N4 -> pp_n4
  | N16 -> pp_n16
...
let[@coverage off] pp_kind : type a. a kind Fmt.t =
 fun ppf -> function
...
let[@coverage off] pp_record : type a. a record Fmt.t =
 fun ppf r ->
  match r.kind with
  | NULL -> Fmt.string ppf "<null>"
  | _ ->
      Fmt.pf ppf
        "{ @[<hov>prefix= %S;@ prefix_length= %d;@ count= %d;@ kind= %a;@ \
         value= @[<hov>%a@];@] }"
        (Bytes.unsafe_to_string r.prefix)
        r.prefix_length r.count pp_kind r.kind (pp_keys ~kind:r.kind) r.keys

let[@coverage off] pp_header ppf (Header record) = pp_record ppf record

let[@coverage off] rec pp_elt pp_value ppf = function
  | Leaf { key; value } ->
      Fmt.pf ppf "{:leaf @[<hov>key= %S;@ value= @[<hov>%a@];@] }" key pp_value
        value
  | Node { header = Header { kind = NULL; _ }; _ } -> Fmt.string ppf "<null>"
  | Node { header; children } ->
      Fmt.pf ppf "{:node @[<hov>hdr= @[<hov>%a@];@ children= @[<hov>%a@];@] }"
        pp_header header
        Fmt.(Dump.array (pp_elt pp_value))
        children
...
let empty_record =
  { prefix = Bytes.empty; prefix_length = 0; count = 0; kind = NULL; keys = () }
  ...
  let record =
    {
      ...
      count = 0;
      kind = N16;
      keys = Bytes.make 16 '\000';
    }
  in
  record

let n16_shift keys n = Bytes.unsafe_blit keys n keys (n + 1) (16 - (n + 1))

let n48 prefix : n48 record =
  let keys = Bytes.make 256 '\048' in
  let record = { prefix; prefix_length = 0; count = 0; kind = N48; keys } in
  record

let n256 prefix : n256 record =
  let record =
    { prefix; prefix_length = 0; count = 0; kind = N256; keys = N256_Key }
  in
  record
...
let copy_header : type a b. src:a record -> dst:b record -> unit =
 fun ~src ~dst ->
  dst.count <- src.count;
  dst.prefix_length <- src.prefix_length

let add_child_n256 : n256 record -> 'a elt array -> char -> 'a elt -> unit =
...
let add_child_n48 :
 ...
 fun record tree children chr node ->
  if record.count < 48 then (
    let pos = ref 0 in
    while Array.unsafe_get children !pos != empty_elt do
      incr pos
    done;
    ...
    let node256 = n256 record.prefix in
    copy_header ~src:record ~dst:node256;
    let children =
      Array.init 256 (fun i ->
          let k = Char.code record.keys.!{i} in
          if k <> 48 then Array.unsafe_get children k else empty_elt)
    in
...
let add_child_n16 :
 ...
 fun record tree children chr node ->
  if record.count < 16 then (
    ...
    let idx = ref 0 in
    for i = 0 to 15 do
      if chr < record.keys.!{i} then bit := !bit lor (1 lsl i)
    done;
    bit := !bit land mask;
    if !bit <> 0 then (
      idx := ctz !bit;
    ...
    let node48 = n48 record.prefix in
    for i = 0 to record.count - 1 do
      node48.keys.!{Char.code record.keys.!{i}} <- Char.unsafe_chr i
    done;
...
let rec iter_child_n4 keys idx max chr =
  if idx < max then
    if Char.code chr <= Char.code keys.!{idx} then idx
...
let add_child_n4 :
 ...
 fun record tree children chr node ->
  if record.count < 4 then (
    let idx = iter_child_n4 record.keys 0 record.count chr in
    ...
    let node16 = n16 record.prefix in
...
let find_child : 'a node -> char -> int =
 fun { header = Header record; _ } chr ->
  ...
  | N4 ->
      ...
      if m > 0 && Char.code record.keys.!{0} = code then res := 0
      else if m > 1 && Char.code record.keys.!{1} = code then res := 1
      else if m > 2 && Char.code record.keys.!{2} = code then res := 2
      else if m > 3 && Char.code record.keys.!{3} = code then res := 3
  | N16 ->
      (* TODO(dinosaure): can be replaced by SSE instr. *)
      let bit = ref 0 in
      for i = 0 to 15 do
        if record.keys.!{i} = chr then bit := !bit lor (1 lsl i)
      done;
      let mask = (1 lsl record.count) - 1 in
...
let check_prefix ~prefix ?(prefix_offset = 0) ~prefix_length ~off key len =
  ...
  let idx = ref 0 in
  while !idx < max && prefix.!{prefix_offset + !idx} = key.![off + !idx] do
    incr idx
  done;
  !idx
...
let rec minimum = function
  ...
  | Node { header = Header { kind = N48; keys; _ }; children } ->
      let idx = ref 0 in
      while keys.!{!idx} = '\048' do
        incr idx
      done;
      idx := Char.code keys.!{!idx};
      minimum (Array.unsafe_get children !idx)
  | Node { header = Header { kind = N256; _ }; children } ->
      let idx = ref 0 in
      while Array.unsafe_get children !idx == empty_elt do
        incr idx
      done;
      minimum (Array.unsafe_get children !idx)
...
let prefix_mismatch ({ header = Header header; _ } as node) ~off key len =
  ...
  let idx = ref 0 in
  while !idx < max && header.prefix.!{!idx} = key.![off + !idx] do
    incr idx
  done;

  if !idx = max && plen > 10 then (
    let leaf = minimum (Node node) in
    let max = min (String.length leaf.key) len - off in
    while
      !idx < max - 4
      && String.unsafe_get_uint32 leaf.key (off + !idx)
         = String.unsafe_get_uint32 key (off + !idx)
    do
      idx := !idx + 4
    done;
    while !idx < max && leaf.key.![off + !idx] = key.![off + !idx] do
      incr idx
    done);
  !idx
...
let longest_common_prefix ~off k1 k2 =
  ...
  let idx = ref 0 in
  while !idx < max && k1.![off + !idx] = k2.![off + !idx] do
    incr idx
  done;
  !idx
...
let rec _find ~key ~key_len depth elt =
  match elt with
  | Leaf leaf ->
      leaf_matches leaf key ~off:depth key_len;
      leaf.value
  | Node { header = Header { kind = NULL; _ }; _ } -> raise Not_found
  | Node ({ header = Header header; children } as node) ->
      let plen = header.prefix_length in
      let depth =
        if plen > 0 && plen <= 10 then (
          let plen' =
            check_prefix ~prefix:header.prefix ~prefix_length:plen ~off:depth
              key key_len
          in
          if plen' <> min 10 plen then raise Not_found;
          depth + plen)
        else if plen > 10 then (
          let prefix = Bytes.unsafe_of_string (minimum elt).key in
          let plen' =
            check_prefix ~prefix ~prefix_offset:depth ~prefix_length:plen
              ~off:depth key key_len
          in
      ...
      if x = not_found || Array.unsafe_get children x == empty_elt then
        raise Not_found
...
let find_opt tree key =
  match find tree key with v -> Some v | exception Not_found -> None

let rec insert tree elt key_a len_a value_a depth =
  match elt with
  ...
  | Node ({ header = Header record; children } as node) ->
      ...
      if pdiff >= plen then (
        ...
        | idx, _ ->
            ...
            insert cur
              (Array.unsafe_get children idx)
              key_a len_a value_a
              (depth + plen + 1);
            Array.unsafe_set children idx !cur)
      else
        let node4 =
          n4 (Bytes.make 10 '\000')
        ...
        (if plen <= 10 then (
           add_child_n4 node4 null children' record.prefix.!{pdiff} elt;
           let plen' = plen - (pdiff + 1) in
           record.prefix_length <- plen';
           Bytes.unsafe_blit record.prefix (pdiff + 1) record.prefix 0
             (min 10 plen'))
         else
           let plen' = plen - (pdiff + 1) in
           record.prefix_length <- plen';
           ...
           Bytes.blit_string bot.key
             (depth + pdiff + 1)
             record.prefix 0 (min 10 plen'));
        add_child_n4 node4 null children'
          key_a.![depth + pdiff]
          (Leaf { key = key_a; value = value_a });
        tree := Node { header = Header node4; children = children' }
  | Leaf leaf -> (
      try
        leaf_matches leaf ~off:depth key_a len_a;
        tree := Leaf { leaf with value = value_a }
      with Not_found ->
        let node4 =
          n4 (Bytes.make 10 '\000')
          (* TODO(dinosaure): check that! *)
...
let insert tree key value = insert tree !tree key (String.length key) value 0
...
let remove_child_n256 :
 ...
 fun record tree children chr ->
  ...
  if record.count = 37 then (
    ...
    let pos = ref 0 in
    for i = 0 to 255 do
      if children.(i) != empty_elt then (
        children'.(!pos) <- children.(i);
        node48.keys.!{i} <- Char.unsafe_chr !pos;
        incr pos)
    done;
    tree := Node { header = Header node48; children = children' })

let remove_child_n48 : n48 record -> 'a elt ref -> 'a elt array -> char -> unit
    =
 fun record tree children chr ->
  ...
  if record.count = 12 then (
    ...
    for i = 0 to 255 do
      ...
      if pos <> 48 then (
        node16.keys.!{!child} <- Char.chr i;
        children'.(!child) <- children.(pos);
        incr child)
    done;
    tree := Node { header = Header node16; children = children' })

let remove_child_n16 : n16 record -> 'a elt ref -> 'a elt array -> int -> unit =
 fun record tree children pos ->
  ...
  for pos = record.count - 1 to 15 do
    children.(pos) <- empty_elt
  done;
  record.count <- record.count - 1;
  if record.count == 3 then (
    let node4 = n4 record.prefix in
    let children' = Array.make 4 empty_elt in
...
let unsafe_get_key : type a. a record -> int -> char =
 fun record n ->
  match record.kind with
...
let remove_child_n4 : n4 record -> 'a elt ref -> 'a elt array -> int -> unit =
 fun record tree children pos ->
  Bytes.blit record.keys (pos + 1) record.keys pos (record.count - 1 - pos);
  Array.blit children (pos + 1) children pos (record.count - 1 - pos);
  for pos = record.count - 1 to 3 do
    children.(pos) <- empty_elt
  done;
  ...
  if record.count = 1 then
    ...
    | Node { header = Header ({ prefix_length; _ } as hdr); _ } as child ->
        ...
        if !prefix < 10 then (
          Bytes.unsafe_set record.prefix !prefix (unsafe_get_key record 0);
          incr prefix);
        if !prefix < 10 then (
          let sub = min prefix_length (10 - !prefix) in
...
let remove_child : 'a node -> 'a elt ref -> char -> int -> unit =
...
let rec remove : 'a elt -> 'a t -> string -> int -> int -> unit =
 fun elt tree key key_len depth ->
  match elt with
  | Node ({ header = Header header; children } as node) -> (
      let plen = header.prefix_length in
      let depth =
...
and go ~prefix ~f acc depth elt =
  ...
  | Node _ when depth = String.length prefix ->
      leaf_prefix_matches (minimum elt) prefix;
      (* XXX(dinosaure): or [Not_found] *)
      iter ~f acc elt
  | Leaf leaf ->
      leaf_prefix_matches leaf prefix;
      (* XXX(dinosaure): or [Not_found] *)
  ...
  | Node ({ header = Header { prefix_length; _ }; children } as node) ->
      let plen =
        prefix_mismatch node ~off:depth prefix (String.length prefix)
      in
      let plen = if plen > prefix_length then prefix_length else plen in
      if plen = 0 then raise Not_found;
      if plen + depth = String.length prefix then iter ~f acc elt
      else
        let x = find_child node prefix.![depth + prefix_length] in
        if x = not_found || Array.unsafe_get children x == empty_elt then
          raise Not_found;
        go ~prefix ~f acc
          (depth + prefix_length + 1)
          (Array.unsafe_get children x)
