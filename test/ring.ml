...
let create ~order filename =
  ...
  let memory =
    Mmap.V1.map_file fd ~pos:0L Bigarray.int Bigarray.c_layout true [| len |]
  in
...
let random_filename =
  ...
  create ~order (Fpath.to_string filename);
  filename
...
let pop ?(non_empty = false) ring =
  rrun ring Ringbuffer.(dequeue ~order ~non_empty zero)

let peek ?(non_empty = false) ring =
  rrun ring Ringbuffer.(peek ~order ~non_empty zero)
...
let filename =
  let parser x =
    ...
    | Ok v when not (Sys.file_exists x) ->
        create ~order (Fpath.to_string v);
        Ok v
...
let () =
  Alcotest.run_with_args "ring" filename
