...
let exists state v key =
  ...
  | state, v' ->
      if v <> v' then (
        Logs.err (fun m -> m "Wrong value for %s." (key :> string));
        exit 1)
      else (
        Logs.debug (fun m -> m "%s => %d." (key :> string) v);
        (true, state))
  | exception Not_found -> (false, state)

type kind =
  [ `Simple_consumer_simple_producer | `Multiple_consumer_simple_producer ]
...
let test ~kind dataset path =
  ...
  let fiber0 () =
    let th0 =
      ...
      go (open_in (Fpath.to_string dataset)) 0
    in
    let _closed, () = Part.(run closed th0) in
    ()
  in
  let fiber1 dataset () =
    let rec go state dataset queue =
      ...
      if not (Array.for_all identity ress) then (
        let _missing =
          let fold acc = function true -> acc | false -> succ acc in
          Array.fold_left fold 0 ress
        in
        ...
        Logs.debug (fun m ->
            m "End of reader: @[<hov>%a@]" Fmt.(Dump.array bool) ress);
        Queue.push ress queue;
        (state, Queue.fold (fun ress x -> x :: ress) [] queue))
    in
  ...
  let dataset =
    ...
    in
    go (open_in (Fpath.to_string dataset)) []
  in
  match kind with
  | `Multiple_consumer_simple_producer -> (
      ...
      let readers () =
        let f _ =
          ...
          run_process ~file:(Fpath.to_string temp) (fiber1 dataset)
          >>= fun _res -> return ()
        in
        parallel_iter ~f (List.init (get_concurrency () - 1) identity)
        >>= fun () -> return (Ok ())
      in
      ...
      | Ok (), Ok () ->
          Fmt.pr ">>> %d reader(s) terminate correctly.\n%!"
            (get_concurrency () - 1);
          return (Ok ())
      | Error exit, _ -> return (R.error_msgf "Reader exits with %03d" exit)
...
let main multiple_readers dataset () () () =
  ...
  let kind =
    match multiple_readers with
    | true -> `Multiple_consumer_simple_producer
    | false -> `Simple_consumer_simple_producer
  in
...
let setup_concurrency v = Fiber.set_concurrency v

let setup_concurrency =
  Term.(
    const setup_concurrency
    $ Arg.(
        value & opt int (Fiber.get_concurrency ()) & info [ "c"; "concurrency" ]))
...
let multiple_readers = Arg.(value & flag & info [ "multiple-readers" ])

let main =
  Cmd.v (Cmd.info "ring")
    Term.(
      term_result
        (const main $ multiple_readers $ dataset $ setup_concurrency $ setup_tmp
       $ setup_logs))
